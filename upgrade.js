 ...
 * @LastEditTime: 2025-08-28 11:58:41
...
const contractsMgrNew = sdk12.contracts_mgr;
const contractsNew = sdk12.contracts
const nftContracts = require("crosschain-sdk-1-2/nft-contract");

const sdkOld = new sdk11.ContractSdk(true);
const sdkNew = new sdk12.ContractSdk(true);
...
// const scriptRefOwnerAddr = 'addr1qys3nr0s5wqz3gw2n9satl279ntzha2z92v4ewrknr234hzx8ugllqwa07adyqwz23j797tha446p0exqa8jjypyqzasq73gym';//'addr_test1vq73yuplt9c5zmgw4ve7qhu49yxllw7q97h4smwvfgst32qrkwupd';//
...
const signatories = [
    ...
    // 'addr_test1qq6qv6py3skvh0zh3dam9nywzvdr5pxvzwvq3kyhfakpmt7ng0vl8m0gl4q6yla5tplxupnxfywn9ntrrff7smaqrahqsnry8d',
    // 'addr_test1qq42tgrmted649d0r8k3gg0cw6jxgx99dzqyux5lhnhxzjefqdqd0646h5lsqtd7ljsgn0lksgg7j6su2kx6w39t67js4lnkfr',
    // 'addr_test1qrpnvwy34q7qrunra7d89sypny44jmcrtqmrz5mz72sh94drdkzsdp4rpxacmr76akxh7c05ha3mkyv8n2vsm5jzsgasfkw0g3',
    // 'addr_test1qqc2y9cw7jd39yffc5epc98qgqhgd4sjw2m2kunhex7zrcz4cnlw0wf7sp9frwfv8q20n2r4gnfcr8sws4ghxydy4f3s6dt0ss'
...
async function getGroupInfo(old = true) {
    if(old){
        const groupInfoTokenUtxo = await sdkOld.getGroupInfoNft();
...
async function getAdminInfo(old = true) {
    if(old){
        const adminNftUtxo = await sdkOld.getAdminNft();
...
async function getCheckTokenUtxo(type, old = true) {
    ...
    switch (type) {
        case 0:
            owner = contractsOld.TreasuryCheckScript.address(stkvh).to_bech32(sdkOld.ADDR_PREFIX);
            tokenId = contractsOld.TreasuryCheckTokenScript.tokenId();
            break;

        case 1:
            owner = contractsOld.MintCheckScript.address(stkvh).to_bech32(sdkOld.ADDR_PREFIX);
...
async function finalTxEvaluate(signedTx) {
    signedTx = await sdkOld.addSignature(signedTx, signFnNext);
    signedTx = await sdkOld.addSignature(signedTx, signFn);
...
async function show(old = true) {
    ...
    console.log('StoremanStackScript Addr:', contractsMgr.StoremanStackScript.script().hash().to_hex(), contractsMgr.StoremanStackScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
    console.log('AdminNFTHolderScript Addr:', contractsMgr.AdminNFTHolderScript.script().hash().to_hex(), contractsMgr.AdminNFTHolderScript.address().to_bech32(sdkOld.ADDR_PREFIX));
    const adminInfoUtxo = await sdkOld.getAdminNft();
    ...
    console.log('GroupInfoNFTHolderScript Addr:', contractsMgr.GroupInfoNFTHolderScript.script().hash().to_hex(), contractsMgr.GroupInfoNFTHolderScript.address().to_bech32(sdkOld.ADDR_PREFIX));
    const groupInfoUtxo = await sdkOld.getGroupInfoNft();
    ...
    console.log('Treasury Addr:', contracts.TreasuryScript.script().hash().to_hex(), contracts.TreasuryScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
    console.log('TreausyCheck Addr:', contracts.TreasuryCheckScript.script().hash().to_hex(), contracts.TreasuryCheckScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
    console.log('MappingToken policy:',contracts.MappingTokenScript.policy_id());
    console.log('MintCheck Addr:', contracts.MintCheckScript.script().hash().to_hex(), contracts.MintCheckScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
    if(!old){
        console.log('NFT ref holder:',nftContracts.NFTRefHolderScript.address().to_bech32(sdkOld.ADDR_PREFIX));
        console.log('NFTTreasury:',nftContracts.NFTTreasuryScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
        ...
        console.log('NFTTreasuryCheck Addr:', nftContracts.NFTTreasuryCheckScript.script().hash().to_hex(), nftContracts.NFTTreasuryCheckScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
        console.log('NFTMintCheck Addr:', nftContracts.NFTMintCheckScript.script().hash().to_hex(), nftContracts.NFTMintCheckScript.address(stakeHash).to_bech32(sdkOld.ADDR_PREFIX));
    }
}
...
async function getDeployCost(){
    ...
    const contractSdk = new sdk12.ContractSdk(false);
    await contractSdk.init(testnetUrl);
    let scriptRefUtxo = await tryScriptRefUtxo(contractsMgrNew.GroupInfoNFTHolderScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);

    scriptRefUtxo = await tryScriptRefUtxo(contractsNew.TreasuryCheckScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);

    scriptRefUtxo = await tryScriptRefUtxo(contractsNew.MintCheckScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);

    scriptRefUtxo = await tryScriptRefUtxo(nftContracts.NFTMintCheckTokenScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);
    scriptRefUtxo = await tryScriptRefUtxo(nftContracts.NFTMintCheckScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);
    scriptRefUtxo = await tryScriptRefUtxo(nftContracts.NFTTreasuryCheckScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);
    scriptRefUtxo = await tryScriptRefUtxo(nftContracts.NFTTreasuryCheckTokenScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);
    scriptRefUtxo = await tryScriptRefUtxo(nftContracts.NFTTreasuryScript.script());
    totalCost += scriptRefUtxo.value.coins*1;
    console.log(scriptRefUtxo.txHash);
    // scriptRefUtxo = await tryScriptRefUtxo(nftContracts.NFTRefHolderScript.script());
    // totalCost += scriptRefUtxo.value.coins*1;
...
async function main() {
    ...
    // const testnetUrl = "https://nodes-testnet.wandevs.org/cardano";
    await sdkOld.init(mainnetUrl);
    // await sdkNew.init(mainnetUrl);
    // await show();
    // console.log('-----------------------');
    // await show(false);
    ...
    // const mintCheckRef = await tryScriptRefUtxo(contracts.MintCheckScript.script());
    {
        ...
        newDatum[contractsMgrNew.GroupNFT.NFTRefHolderVH] = nftContracts.NFTRefHolderScript.script().hash().to_hex();
        newDatum[contractsMgrNew.GroupNFT.NFTTreasuryCheckVH] = nftContracts.NFTTreasuryCheckScript.script().hash().to_hex();
        newDatum[contractsMgrNew.GroupNFT.NFTMintCheckVH] = nftContracts.NFTMintCheckScript.script().hash().to_hex();
        newDatum = contractsMgrNew.GroupNFT.genGroupInfoDatum(newDatum).to_hex();
        let signedTx = await sdkOld.upgradeGroupNFTHolder(newGroupNFTHolder, newDatum, mustSignBy, utxosForFee, [collateralUtxo], admin);
        console.log('--%%%%%%%%%%%%%1-------\n', signedTx.to_json());
        const exUnit =  await ogmiosUtils.evaluateTx(signedTx);
        signedTx = await sdkOld.upgradeGroupNFTHolder(newGroupNFTHolder, newDatum, mustSignBy, utxosForFee, [collateralUtxo], admin, /*signFn*/undefined, exUnit);
        console.log('--%%%%%%%%%%%%%1-------\n', signedTx.to_json());
        console.log('--%%%%%%%%%%%%%2-------\n', signedTx.to_hex());
